...
CREATE TABLE feed_configs (
  ...
  cpo_id TEXT REFERENCES cpos(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  ...
  reconciliation_interval_minutes INTEGER NOT NULL DEFAULT 2,
...
CREATE TABLE availability_snapshots (
  ...
  PRIMARY KEY (charge_point_id, recorded_at)
...
CREATE TABLE price_snapshots (
  ...
  PRIMARY KEY (tariff_id, recorded_at)
) PARTITION BY RANGE (recorded_at);
